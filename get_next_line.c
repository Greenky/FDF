...
static int		re_malloc(char **line, int n, char *arr, int len)
{
	int		c;
	char	*buff;
...
static int		signaturka(char **line, char **tail)
{
	int	i;
	int len;
...
static t_file	*add_elem(t_file *lst, const int fd)
{
	t_file	*buff;
	t_file	*pepe;
...
static int		reader(t_file *lst, char **line, char *arr)
{
	int c;
	int i;
